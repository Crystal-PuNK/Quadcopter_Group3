...
void MPU6050_init(void)
{
	...
	//������ �Բⲻʹ�ܣ��������ѡ�� ±2000º/s
	GY86_WriteRegister(MPU6050_ADDRESS,MPU6050_GYRO_CONFIG ,0x00);
	//���ٶ� �Բⲻʹ�ܣ��������ѡ�񣬸�ͨ�˲��� ±8g
...
void GY86_GetData(void)
{
	...
	GY86DataList.GX = (((int16_t)(DataH << 8) | DataL)/(32768.0/250.0) - params_ram.gyro_offset[0]) * params_ram.gyro_scale[0] * (PI / 180.0f);
	
	DataH = GY86_ReadRegister(MPU6050_ADDRESS,MPU6050_GYRO_YOUT_H);
	DataL = GY86_ReadRegister(MPU6050_ADDRESS,MPU6050_GYRO_YOUT_L);
	GY86DataList.GY = (((int16_t)(DataH << 8) | DataL)/(32768.0/250.0) - params_ram.gyro_offset[1]) * params_ram.gyro_scale[1] * (PI / 180.0f);
	
	DataH = GY86_ReadRegister(MPU6050_ADDRESS,MPU6050_GYRO_ZOUT_H);
	DataL = GY86_ReadRegister(MPU6050_ADDRESS,MPU6050_GYRO_ZOUT_L);
	GY86DataList.GZ = (((int16_t)(DataH << 8) | DataL)/(32768.0/250.0) - params_ram.gyro_offset[2]) * params_ram.gyro_scale[2] * (PI / 180.0f);
