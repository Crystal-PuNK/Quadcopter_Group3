...
/*引脚配置*/
#define OLED2_W_SCL(x)		GPIO_WriteBit(GPIOC, GPIO_Pin_0, (BitAction)(x))
#define OLED2_W_SDA(x)		GPIO_WriteBit(GPIOC, GPIO_Pin_1, (BitAction)(x))

/*引脚初始化*/
void OLED2_I2C_Init(void)
{
	//初始化PC0和PC1作为模拟SCL与SDA
...
{      	
	...
	for (i = 0; i < 8; i++)
	{
		OLED2_WriteData(OLED2_F08x16[Char - ' '][i]);			//显示上半部分内容
	}
	OLED2_SetCursor((Line - 1) * 2 + 1, (Column - 1) * 8);	//设置光标位置在下半部分
	for (i = 0; i < 8; i++)
	{
		OLED2_WriteData(OLED2_F08x16[Char - ' '][i + 8]);		//显示下半部分内容
	}
}
